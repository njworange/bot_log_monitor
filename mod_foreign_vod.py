...
class ModuleForeignVod(PluginModuleBase):

    def __init__(self, P):
        super(ModuleForeignVod, self).__init__(P, name='foreign_vod', first_menu='list')
        self.db_default = {
            f'{self.name}_db_version' : '2',
            'foreign_vod_remote_path' : '',
            'foreign_vod_download_mode' : 'none', #Nothing, 모두받기, 블랙, 화이트
            'foreign_vod_blacklist_genre' : '',
            'foreign_vod_blacklist_program' : '',
            'foreign_vod_blacklist_title' : '',
            'foreign_vod_whitelist_genre' : '',
            'foreign_vod_whitelist_program' : '',
            'foreign_vod_whitelist_title' : '',
            'foreign_vod_watch_paths' : '/ROOT/GDRIVE/VIDEO/방송중/외국/\n/ROOT/GDRIVE/VIDEO/외국TV',  # 감시 경로 규칙 (줄바꿈으로 구분)
            'foreign_vod_item_last_list_option': '',
            f'{self.name}_db_delete_day': '30',
            f'{self.name}_db_auto_delete': 'False',
            'foreign_vod_use_notify': 'False',
        }
        self.web_list_model = ModelForeignVodItem
    ...
    def process_command(self, command, arg1, arg2, arg3, req):
        ...
        if command == 'option':
            mode = arg1
            value = arg2
            value_list = P.ModelSetting.get_list(f'foreign_vod_{mode}', '|')
            if value in value_list:
                ret['ret'] = 'warning'
                ret['msg'] = '이미 설정되어 있습니다.'
            else:
                if len(value_list) == 0:
                    P.ModelSetting.set(f'foreign_vod_{mode}', value)
                else:
                    P.ModelSetting.set(f'foreign_vod_{mode}', P.ModelSetting.get(f'foreign_vod_{mode}') + ' | ' + value)
                ret['msg'] = '추가하였습니다'
        elif command == 'request_copy':
            item = ModelForeignVodItem.get_by_id(arg1)
    ...
    def process_log_data(self, data):
        ...
        item = ModelForeignVodItem.process_discord_data(data)
        if item is None:
            P.logger.debug(f'외국 VOD 스킵 (이미 존재): {filename}')
            return
        try:
            P.logger.debug(f'외국 VOD 필터링 체크: {item.filename}')
            flag_download = self.condition_check_download_mode(item)
            P.logger.debug(f'외국 VOD 필터링 결과: {flag_download} (로그: {item.log})')
            
            if flag_download:
                P.logger.info(f'외국 VOD 복사 요청: {item.filename}')
                result = self.share_copy(item)
                P.logger.debug(f'외국 VOD 복사 결과: {result}')
                ...
                P.logger.info(f'외국 VOD 필터링 제외: {item.filename}')
                
            if P.ModelSetting.get_bool('foreign_vod_use_notify'):
    ...
    def share_copy(self, item):
        try:
            foreign_vod_remote_path = P.ModelSetting.get('foreign_vod_remote_path')
            if foreign_vod_remote_path == '':
            ...
            # 파일 ID만으로 직접 복사
            ret = PP.add_copy(item.fileid, None, 'bot_log_monitor_foreign_vod', item.meta_genre, item.size, 1, copy_type='file', remote_path=foreign_vod_remote_path)
    ...
    def share_folder_copy(self, item):
        """프로그램 폴더 전체 복사"""
        try:
            P.logger.info(f'외국 VOD 폴더 복사 요청 시작: {item.filename}')
            ...
            # 예: gds_path = .../메이어 오브 킹스타운 (2021)/Season 3/파일.mkv
            #     program_folder = 메이어 오브 킹스타운 (2021)
            #     folder_path = .../메이어 오브 킹스타운 (2021)
            ...
            P.logger.info(f'외국 VOD 폴더 복사 요청: {program_folder}')
            ...
            ret = PP.add_copy(
                folder_id, 
                program_folder, 
                'bot_log_monitor_foreign_vod_folder', 
                item.meta_genre, 
                folder_size, 
                folder_count, 
                copy_type='folder', 
                remote_path=foreign_vod_remote_path
            )

            if ret['ret'] == 'success':
                # 디스코드 알림
                ModuleMonitor.send_discord_notification(
                    title="📁 외국 VOD 폴더 복사 요청",
    ...
    def condition_check_download_mode(self, item):
        try:
            ...
            foreign_vod_download_mode = P.ModelSetting.get('foreign_vod_download_mode')
            if foreign_vod_download_mode == 'none':
                return False
            if foreign_vod_download_mode == 'blacklist':
                flag_download = True
                foreign_vod_blacklist_genre = P.ModelSetting.get_list('foreign_vod_blacklist_genre', '|')
                foreign_vod_blacklist_program = P.ModelSetting.get_list('foreign_vod_blacklist_program', '|')
                foreign_vod_blacklist_title = P.ModelSetting.get_list('foreign_vod_blacklist_title', '|')
                if item.meta_title is None:
                    item.log += u'메타 정보 없음. '
                if item.meta_title is not None and len(foreign_vod_blacklist_genre) > 0 and item.meta_genre in foreign_vod_blacklist_genre:
                    flag_download = False
                    item.log += '제외 장르. 다운:Off'
                if flag_download and program_text:
                    for program_name in foreign_vod_blacklist_program:
                        if program_text.find(program_name.replace(' ', '').lower()) != -1:
                            flag_download = False
                            item.log += '제외 프로그램. 다운:Off'
                            break
                if flag_download and title_text:
                    for title_name in foreign_vod_blacklist_title:
                ...
                flag_download = False
                foreign_vod_whitelist_genre = P.ModelSetting.get_list('foreign_vod_whitelist_genre', '|')
                foreign_vod_whitelist_program = P.ModelSetting.get_list('foreign_vod_whitelist_program', '|')
                foreign_vod_whitelist_title = P.ModelSetting.get_list('foreign_vod_whitelist_title', '|')
                if item.meta_title is None:
                    item.log += '메타 정보 없음. '

                if item.meta_title is not None and len(foreign_vod_whitelist_genre) > 0 and item.meta_genre in foreign_vod_whitelist_genre:
                    flag_download = True
                    item.log += '포함 장르. 다운:On'
                if flag_download == False and program_text:
                    for program_name in foreign_vod_whitelist_program:
                        if program_text.find(program_name.replace(' ', '').lower()) != -1:
                            flag_download = True
                            item.log += '포함 프로그램. 다운:On'
                            break
                if flag_download == False and title_text:
                    for title_name in foreign_vod_whitelist_title:
                        if title_text.find(title_name.replace(' ', '').lower()) != -1:
                            flag_download = True
...
class ModelForeignVodItem(ModelBase):
    P = P
    __tablename__ = 'bot_log_monitor_foreign_vod_item'
